...
SELECT COUNT(DISTINCT(id_student)) AS 'Iloœæ studentów' FROM egzaminy;
...
VALUES
	('Jan', 'Kowalski', 'ul. Wêglowa', '29/87', 'Szczecin', '12345678901');
...
-- Ile osób zatrudnia firma = Liczba wszystkich pracowników
SELECT COUNT(FirstName) AS 'People employed'
FROM Person.Person
WHERE Person.PersonType = 'EM';

-- Nazwa klienta/kontrahenta oraz liczbê z³o¿onych przez niego zamówieñ
...
SELECT p.FirstName, p.LastName, COUNT(s.BusinessEntityID) AS 'Iloœæ zamówieñ'
FROM Sales.SalesPersonQuotaHistory s
JOIN Person.Person p
ON s.BusinessEntityID = p.BusinessEntityID
GROUP BY s.BusinessEntityID, p.FirstName, p.LastName;

-- Wyœwietl nazwy stanowisk na które s¹ zatrudnieni pracownicy
SELECT * FROM HumanResources.Department
SELECT * FROM HumanResources.EmployeeDepartmentHistory ORDER BY DepartmentID
-- wrong, this select all departments where someone work. department = dzia³, stanowisko to coœ innego
...
-- Oblicz najwiêksz¹ i najmniejsz¹ wartoœæ zamówienia
SELECT MIN(TotalPrice) AS 'Najmniejsza wart. zamówienia',
	   MAX(TotalPrice) AS 'Najwiêksza wart. zamówienia'
FROM
